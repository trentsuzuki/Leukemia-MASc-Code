...
f(x) = (Base/x)^gamma;
mu(x) = C1*C5*(x-x_d)/(k_tr*(C5*x - a4*x_d));
g(x) = 1/slope*(1 - (mu(x) + 1)/f(x));
h(x) = 1/(FM_3*k_me)*(CL_6tgn*g(x) + diff(g(x))*C1*(x-x_d));
q(x) = 1/k_a*(k_20*h(x) + diff(h(x))*C1*(x-x_d));
u(x) = 1/F*(k_a*q(x) + diff(q(x))*C1*(x-x_d));
...
dataSheet1Path = "C:\Users\trent\UofT\Research\Leukemia\Code\Jost 2020 Data Analysis\Supplementary Materials\Data Sheet 1.CSV";
...
dataSheet2Path = "C:\Users\Trent\UofT\Research\Leukemia\Code\Jost 2020 Data Analysis\Supplementary Materials\Data Sheet 2.CSV";
...
numInitConds = 5;
randomStateUpperLims = [10;10;10;60;60;60;60;7];
% initConds = randomStateUpperLims.*rand(8,numInitConds);
...
for i = 1:numPatients
    ...
    u_nom(x) = vpa(subs(max(u(x),0), fieldnames(params), struct2cell(params)),3);

    patients(i).TargetANC = params.x_d;
    patients(i).ControlLaw = u_nom;
    ...
    for j = 1:numInitConds
        ...
        [t_sim,x_sim] = ode45(@(t,x) JostModelWithParams(t,x,u_nom(x(8)),params), [0 Tmax], x0);
...
save("results/BacksteppingMultiStart_" + current_time, 'patients')
