...
% Operating point for observer
x3_op = 0.5;
x4_op = 0.4;
x8_op = 1.5;

% Construct the Jacobian matrix A at the operating point
A = zeros(8);
A(1,1) = -k_a;
...
A(4,3) = -k_tr * x4_op * slope * (Base / x8_op)^gamma;
A(4,4) = k_tr * ((1 - slope * x3_op) * (Base / x8_op)^gamma - 1);
A(4,8) = -k_tr * x4_op * (1 - slope * x3_op) * gamma * Base^gamma * x8_op^(-gamma - 1);
...
%% Substitute parameters into observer

A = subs(A, symbols(:,1), symbols(:,2));
B = subs(B, symbols(:,1), symbols(:,2));
C = subs(C, symbols(:,1), symbols(:,2));

% Compute observer gain L using pole placement
L = place(A', C', desired_poles)';

% Simulation parameters
tspan = [0 20]; % Simulation time
z0_est = zeros(8, 1); % Observer initial estimate

% Input function (example: step input at t=5)
u = @(t) 1.0 * (t >= 5);

% Combined ODE function for true system and observer
observer_ode = @(t, xz) [
    A * xz(9:16) + B * u(t) - L * (C * xz(9:16) - C * xz(1:8))  % Observer dynamics
];

% Solve the ODE
[time, xz] = ode45(observer_ode, tspan, [x0; z0_est]);
...
figure;
subplot(2,1,1);
plot(time, x_true(:,1), 'b', time, z_est(:,1), 'r--');
...
subplot(2,1,2);
plot(time, x_true(:,4), 'b', time, z_est(:,4), 'r--');
title('State 4 Estimation');
xlabel('Time');
ylabel('State Value');
legend('True x_4', 'Estimated x_4');
