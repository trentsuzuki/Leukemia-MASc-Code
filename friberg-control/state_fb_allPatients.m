...
subplot(2,1,1);
hold on;
title(['State Feedback Control: All Patients  (', ...
...
for p_idx = 1:height(patientData) 
    ...
    for i = 1:num_simulations
        ...
        % Define the (saturated) state feedback control law
        u = @(x) max(0, 1 - (x(4,:) * y_r)./(x(5,:) .* x(1,:)) .* (x(5,:)./Base).^gamma);
...
if save_trajectory_figures
    trajectory_filename = 'figs/State_fb_All_Patients_Combined_Trajectories.png';
