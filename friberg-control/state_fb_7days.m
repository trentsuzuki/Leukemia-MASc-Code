...
% simulations for each patient with a state feedback control input
...
for p_idx = 1:height(patientData) 
    ...
    subplot(2,1,1);
    hold on;
    title(sprintf('Patient %d: State Feedback Control (%d-Day Updates)', patient_id, measurement_interval), ...
        'FontSize', 16);
    xlabel('Time (days)');
    ...
    trajectory_filename = sprintf('figs/7Day_State_fb_Patient_%d_Trajectories_Periodic.png', patient_id);
