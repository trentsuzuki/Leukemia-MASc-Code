...
ny = 1;
nu = 1;
np = 3; % Number of parameters to estimate (Base, slope, gamma)
...
% Process noise for the 3 estimated parameters [Base, slope, gamma]
ukfParamProcessNoiseCovs = [1e-4 1e-4 1e-4];
ukfInitialStateSDs = [1e-2*ones(1,3) 0.125304565*ones(1,4) 0.1036877
% Initial SD for the 3 estimated parameters [Base, slope, gamma]
ukfInitialParamSDs = [0.103687761 0.161059312 0.044632609]; % SD of log-params from Jost Data Sheet 1
...
parfor patientIdx = 1:numPatients
    ...
    % k_tr is assumed to be known, so set it to its true value
    phat.k_tr = patientKtr(patientIdx);
    ...
    % Initial parameter estimates now only include Base, slope, gamma
    zhat0(nx+1:nx+np) = [phat.Base; phat.slope; phat.gamma];
...
%% Save results
save("results/ga_ukf_3param_est_" + string(datetime("now"),"yyyyMMdd_hhmm"))
...
paramNames = {'Base', 'slope', '\gamma'};
errorFields = {'Base', 'slope', 'gamma'};
figure('Position', [200 200 1200 400]);

for paramIdx = 1:np
    subplot(1,3,paramIdx);
    hold on;
    for i = 1:numPatients
        if isempty(allResults{i})
            continue
...
for i = 1:numPatients
    ...
    paramEstErrors.slope(i, :) = (estimatedParams(:, 2) - trueParams.slope) / trueParams.slope * 100;
    finalErrors.slope(i) = paramEstErrors.slope(i, end);
    paramEstErrors.gamma(i, :) = (estimatedParams(:, 3) - trueParams.gamma) / trueParams.gamma * 100;
...
function xnext = jost_integrator(x,u,p,Delta)
    ...
    options = odeset('RelTol',1e-1, 'AbsTol',1e-2, 'NonNegative', 1:length(x));
...
% This function builds the struct from a 3-element parameter
% vector (pvec) and uses the known k_tr from the base struct (p).
function pstruct = makeParamStruct(p,pvec)
    ...
    pstruct.k_ma = p.k_ma;
    % Get estimated parameters from pvec
    pstruct.Base = pvec(1);
    pstruct.slope = pvec(2);
    pstruct.gamma = pvec(3);
    % Get known parameter from the base struct p
    pstruct.k_tr = p.k_tr;
end
