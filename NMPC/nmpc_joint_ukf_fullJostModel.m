%% Nonlinear model predictive control using Joint Unscented Kalman Filter
...
mpcAncWeight = 100; % Weight for ANC
...
ukfParamProcessNoiseCovs = [1e-3 1e-3 1e-3 1e-3];
...
for patientIdx = 1%:numPatients
    ...
    mpcStateFcn = @(z,u) [
        ...
        z(nx+1:end) % parameters assumed are constant
    ];
    mpcOutputFcn = @(z,u) exp(z(nx)); % measurement is ANC only
    ...
    jointUkf = unscentedKalmanFilter( ...
        mpcStateFcn, ...
        @(z) z(nx), ... % Measure in log domain to preserve measurement noise calc.
...
targetTolerance = 0.3 * mpcAncTarget;
...
%     end
% 
%     dt = 1/48; % e.g. 1/24 = 1 hour time step
%     Nt = round(Delta/dt);
% 
...
% 
%     for n = 1:Nt
%         % Check for dose event (start of a new day)
%         if mod((n-1)*dt,1) == 0
%             % Apply the instantaneous dose to C (x(1))
%             x(1) = x(1) + deltaC;
%         end
% 
%         % Integrate the system dynamics over dt with zero drug input.
%         x = rk4(@(z) jost_rhs(z,0,p), x, dt);
% 
...
function xnext = jost_integrator(x,u,p,Delta)
    ...
    options = odeset('RelTol',1e-6, 'AbsTol',1e-8, 'NonNegative', 1:length(x));
    ...
    for i = 1:length(tspan)-1
        ...
        % TODO: Replace ode45 with another solver?
        [~, Y] = ode15s(@(t,y) jost_rhs(y, 0, p), [tspan(i) tspan(i+1)], xnext, options);
