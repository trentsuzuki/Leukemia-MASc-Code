...
mpcDoseWeight = 1e-3; % Weight for 6-MP dose
...
ukfParamProcessNoiseCovs = [1e-3 1e-3 1e-3 1e-2];
...
parfor patientIdx = 1:numPatients
    ...
    jointUkf = unscentedKalmanFilter( ...
        mpcStateFcn, ...
        @(x) x(nx), ... % Measure in log domain to preserve measurement noise calc.
    ...
    % Initialization
    x0 = [1e-3; p.Base*p.k_ma/p.k_tr*ones(4,1); p.Base];
    
    zhat0 = nan(nx+np,1);
    zhat0(1:nx) = [1e-3; phat.Base*phat.k_ma/phat.k_tr*ones(4,1); phat.Base];
    ...
    for k = 1:Nk
        ...
        currentParamEst = exp(jointUkf.State(nx+1:end));
        ...
        try
            jointUkf.State(nx+1:end) = log(currentParamEst);
        catch ME
...
function xnext = friberg_integrator(x,u,p,Delta)
    ...
    dt = 1/24; % e.g. 1/24 = 1 hour time step
    Nt = round(Delta/dt);
