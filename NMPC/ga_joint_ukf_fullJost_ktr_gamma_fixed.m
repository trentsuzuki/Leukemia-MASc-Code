...
% This version assumes k_tr and gamma are known for each patient.
...
% Process noise for the 2 estimated parameters [Base, slope]
ukfParamProcessNoiseCovs = [1e-4 1e-4];
ukfInitialStateSDs = [1e-2*ones(1,3) 0.125304565*ones(1,4) 0.103687761];
% Initial SD for the 2 estimated parameters [Base, slope]
ukfInitialParamSDs = [0.103687761 0.161059312]; % SD of log-params from Jost Data Sheet 1
...
parfor patientIdx = 1:numPatients
    ...
    % k_tr and gamma are assumed to be known, so set them to their true values
    phat.k_tr = patientKtr(patientIdx);
    phat.gamma = patientGamma(patientIdx);
    ...
    zhat0(nx+1:nx+np) = [phat.Base; phat.slope];
...
%% Save results
save("results/ga_ukf_2param_est_" + string(datetime("now"),"yyyyMMdd_hhmm"))
...
paramNames = {'Base', 'slope'};
errorFields = {'Base', 'slope'};
figure('Position', [200 200 800 400]);
for paramIdx = 1:np
    subplot(1,2,paramIdx);
    hold on;
    for i = 1:numPatients
        if isempty(allResults{i})
            continue
...
% vector (pvec) and uses the known k_tr and gamma from the base struct (p).
function pstruct = makeParamStruct(p,pvec)
    ...
    % Get known parameters from the base struct p
    pstruct.k_tr = p.k_tr;
    pstruct.gamma = p.gamma;
end
